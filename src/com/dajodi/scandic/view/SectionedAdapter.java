...
/**
 * Allows sections to be added to a list view.  We can use this for a per-month sectioned view.
 * 
 * @author jon
 *
 */
abstract public class SectionedAdapter extends BaseAdapter {
	...
	public Object getItem(int position) {
		for (Section section : this.sections) {
			...
			// returning the section is useless, but oh well
			if (position==0) {
				return(section);
			}
	...
	public int getViewTypeCount() {
		...
		// this is not ideal, it should cache the type of the adapter, so not to include
		// duplicates
		for (Section section : this.sections) {
